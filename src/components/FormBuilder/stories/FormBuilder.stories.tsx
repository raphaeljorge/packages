...
const CustomRowWrapper: React.FC<WrapperProps> = ({ children, id }) => (
  <div className="mb-8 border border-gray-200 p-4 rounded-lg">
    <div className="mb-2 font-semibold text-gray-700">Row: {id}</div>
    {children}
  </div>
);

const _CustomColumnWrapper = ({ children }: WrapperProps) => <div className="p-2">{children}</div>;
...
const ImportantFieldWrapper: React.FC<WrapperProps> = ({ children, id }) => (
  <div className="p-4 border-2 border-red-500 rounded-lg bg-red-100">
    <div className="font-bold text-red-700 mb-2">Important Field: {id}</div>
    {children}
  </div>
);

// Side by side wrapper for displaying fields in a row
const SideBySideWrapper: React.FC<WrapperProps> = ({ children }) => (
  <div className="flex flex-row gap-4 w-full md:flex-row sm:flex-col">{children}</div>
);
...
const FormBuilderExample = () => {
  ...
  const { state, formState, handleSubmit, resetForm } = form;
