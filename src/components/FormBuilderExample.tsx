...
const CustomRowWrapper: React.FC<WrapperProps> = ({ children, id }) => (
  <div className={classNames('mb-8 border border-gray-200 p-4 rounded-lg')}>
    <div className={classNames('mb-2 font-semibold text-gray-700')}>Row: {id}</div>
    {children}
  </div>
);
...
const CustomColumnWrapper = ({ children, id }: WrapperProps) => (
  <div className={classNames('p-2')}>{children}</div>
);
...
const ImportantFieldWrapper: React.FC<WrapperProps> = ({ children, id }) => (
  <div className={classNames('p-4 border-2 border-red-500 rounded-lg bg-red-100')}>
    <div className={classNames('font-bold text-red-700 mb-2')}>Important Field: {id}</div>
    {children}
  </div>
);
...
const SideBySideWrapper: React.FC<WrapperProps> = ({ children, id }) => (
  <div
    className={classNames(
      ...
      'flex flex-row gap-4 w-full md:flex-row sm:flex-col'
    )}
  >
    {children}
  </div>
    ...
    },
    {
      ...
      columns: [
        ...
        },
        {
          ...
          label: 'Interests',
          required: true,
          options: ['Technology', 'Sports', 'Music', 'Art', 'Travel', 'Food', 'Fashion', 'Science'],
          minItems: 2,
          maxItems: 5,
        },
      ],
...
const FormBuilderExample: React.FC = () => {
  ...
  return (
    <div className={classNames(styles.formExample, 'max-w-4xl mx-auto p-8')}>
      <h1 className={classNames(styles.formTitle, 'text-2xl font-bold mb-8 text-gray-800')}>
      ...
      <div
        className={classNames(
          styles.formDebug,
          'mt-12 p-6 border border-gray-200 rounded-lg bg-gray-50'
        )}
      >
        <h2 className={classNames(styles.debugTitle, 'text-xl font-semibold mb-4 text-gray-800')}>
          Form State
        </h2>
        <div className={classNames('flex gap-8 flex-col md:flex-row')}>
          <div className={classNames('flex-1')}>
            ...
            <pre
              className={classNames(styles.debugPre, 'bg-gray-100 p-4 rounded-md overflow-x-auto')}
            >
          ...
          <div className={classNames('flex-1')}>
            ...
            <pre
              className={classNames(styles.debugPre, 'bg-gray-100 p-4 rounded-md overflow-x-auto')}
            >
              {JSON.stringify(state.masked, null, 2)}
            </pre>
          </div>
        </div>

        <h2
          className={classNames(styles.debugTitle, 'text-xl font-semibold mb-4 mt-8 text-gray-800')}
        >
          Submission State
        </h2>
        <p className={classNames('text-sm mb-4')}>
          Status: {mutation.isPending ? 'Submitting...' : mutation.isSuccess ? 'Success' : 'Idle'}
        </p>
        ...
        <pre className={classNames(styles.debugPre, 'bg-gray-100 p-4 rounded-md overflow-x-auto')}>
